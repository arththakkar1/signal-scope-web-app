"""Fine-tune a ResNet-18 on CIFAKE + julienlucas datasets (EXP-002 v2)."""
...
def train(epochs: int, batch_size: int, max_train_samples: int | None) -> dict[str, float]:
    ...
    cifake_dataset = load_dataset(CIFAKE_DATASET_ID, cache_dir=str(cifake_cache))
    ...
    test_loader_cifake = make_loader(cifake_dataset["test"], evaluation_transform(), batch_size, shuffle=False)
    test_loader_jl = make_loader(jl_test, evaluation_transform(), batch_size, shuffle=False)

    best_validation_auc = -1.0
    output_path = checkpoint_path().with_name("cifake_resnet18_v2.pt")
    ...
    cifake_test_metrics = evaluate(model, test_loader_cifake, device)
    jl_test_metrics = evaluate(model, test_loader_jl, device)
    ...
    report_path = project_root / "report" / "model-report" / "resnet18_v2_metrics.json"
